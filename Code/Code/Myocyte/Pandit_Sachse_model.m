% function [M1]=Pandit_Sachse
% MODEL NAME: Pandit_Sachse in rat
...
% active fibroblasts from Sachese (Sachse et al. 2008).
...
while time<=endtime
    ...
    if(time > (stims1*(xns-1)+tbegin))&& (time < (stims1*(xns-1)+sdur+tbegin))
        AP90=max(APV)-(max(APV)-min(APV))*0.5;
    end
...
AA1 =[st; sv;svf;sCa]';
dlmwrite('Pandit_Sachse.txt',AA1,'delimiter','\t','precision',6)
t2 = clock;
